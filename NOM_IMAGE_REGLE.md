...
| `Canapé convertible` | `canape_convertible.png` |
| `Lit de bébé` | `lit_de_bebe.png` |
| `Machine à laver` | `machine_a_laver.png` |
| `Four micro-ondes` | `four_micro_ondes.png` |

## 🔗 Mots Composés - Règle Universelle

Pour **tous les mots composés**, suivez ces règles simples :

### Principe général
**Chaque mot devient un segment séparé par un underscore** (`_`)

| Type | Exemple | Transformation |
|------|---------|---------------|
| Mot simple | `Climatisation` | `climatisation` |
| 2 mots | `Enceinte bluetooth` | `enceinte_bluetooth` |
| 3 mots | `Jeux de société` | `jeux_de_societe` |
| Avec accent | `Canapé convertible` | `canape_convertible` |
| Avec article | `Table à langer` | `table_a_langer` |
| Avec tiret | `Four micro-ondes` | `four_micro_ondes` |
| Nom propre | `Box Canal+` | `box_canal` |

### Étapes de transformation

1. **Tout en minuscules** : `Canapé Convertible` → `canapé convertible`
2. **Supprimer accents** : `canapé convertible` → `canape convertible`
3. **Remplacer caractères** : `canape convertible` → `canape_convertible`
   - Espace → `_`
   - Tirets (-, –, —) → `_`
   - Caractères spéciaux (+, @, #, etc.) → supprimés
4. **Nettoyer** : `canape__convertible` → `canape_convertible`

### Règle mémorisable

**"Sous-machin truc" → "sous_machin_truc"**

Remplacez TOUS les espaces, tirets, accents et caractères spéciaux par des underscores, en minuscules !
